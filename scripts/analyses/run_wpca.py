"""Run the wpca analysis for sk-nrsr-data-2023-202x.

Usage:
  python scripts/analyses/run_wpca.py \\
      --script /tmp/legislature-data-analyses/wpca/wpca.py \\
      --flourish-script /tmp/legislature-data-analyses/wpca/outputs/output_flourish_table.py
"""
...
_OUTPUT_CSV  = _REPO_ROOT / "analyses/wpca/outputs/wpca_flourish_table.csv"
...
def main() -> None:
    ...
    # wpca uses --vote-events (hyphen), not underscore
    ...
    subprocess.run([
        ...
        "--votes", args.votes,
        "--vote-events", args.vote_events,  # wpca uses hyphen
    ...
    logging.info("WPCA analysis done. Output: %s", output)
